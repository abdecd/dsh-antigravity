...
这是一个 DSH Web 插件。它在 provider 路由 `antigravity` 下注册 DSH `LlmAdapter`，OAuth 凭证存储于 DSH home 目录下，直接与 Cloud Code Assist 流式 API 通信，并且 Web 插件页面完整支持中英文双语国际化（i18n）。
...
打开 **设置（Settings）> 插件（Plugins）> 插件配置 > Antigravity** 并点击 **登录（Login）**。插件页面将发起 Google OAuth 授权流程，并在登录完成后自动刷新配额（Quota）。

![Antigravity Settings - 未登录](./assets/images/settings-not-signed-in.png)

登录成功后，插件页面将展示账号信息、按模型分组的实时配额进度条、重置倒计时以及模型选择器：
...
选择 Antigravity 模型后，输入框模型选择器旁还会显示紧凑的额度条；悬停即可查看剩余额度百分比和重置时间。
...
登录后，在 DSH 模型选择器中选择 **Antigravity** 提供商。您可以在 **设置 > 插件 > 插件配置 > Antigravity** 的模型选择器中按需勾选或关闭单个模型（已勾选的模型将自动置顶展示），每个模型副标题均会显示其实时可用额度百分比。
