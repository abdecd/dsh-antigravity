...
const ANTIGRAVITY_SETTINGS_NAMESPACE = "antigravity";
const ANTIGRAVITY_SETTINGS_NS = ANTIGRAVITY_SETTINGS_NAMESPACE;
const AntigravitySettingsSchema = z.object({});
...
const MODEL_CATALOG_CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000;
...
const inFlightModelCatalogLookups = new Map();
...
function buildModelMatchRegex(requestedId) {
  ...
  if (requested === "gemini-3.8-flash-tiered") return /gemini[- ]3\.8[- ]flash/i;
...
async function fetchMergedAvailableModels(token, projectId) {
  ...
  for (const result of results) {
    ...
    if (isRecord(result.data) && isRecord(result.data.models)) {
      for (const [modelId, info] of Object.entries(result.data.models)) {
        if (!isRecord(info)) continue;
        mergedModels[modelId] = mergeAvailableModelInfo(mergedModels[modelId], info);
      }
    }
...
async function fetchCachedAvailableModels(token, projectId, force = false) {
  const cacheKey = `${token}::${projectId}`;
  const cached = modelCatalogCache.get(cacheKey);
  if (!force && cached && cached.expiresAt > Date.now()) return cached.result;

  const inFlight = inFlightModelCatalogLookups.get(cacheKey);
  if (inFlight) return inFlight;

  const promise = fetchMergedAvailableModels(token, projectId).then((result) => {
    modelCatalogCache.set(cacheKey, { result, expiresAt: Date.now() + MODEL_CATALOG_CACHE_TTL_MS });
    return result;
  });
  inFlightModelCatalogLookups.set(cacheKey, promise);
  try {
    return await promise;
  } finally {
    inFlightModelCatalogLookups.delete(cacheKey);
    if (modelCatalogCache.size > 32) {
      const now = Date.now();
      for (const [key, entry] of modelCatalogCache) {
        if (entry.expiresAt <= now) modelCatalogCache.delete(key);
      }
    }
  }
}

async function fetchQuotaSummaryFromEndpoint(endpoint, token) {
  try {
...
function mergeQuotaBucket(left, right) {
  const leftRemaining = clampFraction(left?.remainingFraction);
  const rightRemaining = clampFraction(right?.remainingFraction);
  if (leftRemaining === undefined && rightRemaining === undefined) return { ...left, ...right };
  if (leftRemaining === undefined) return { ...left, ...right, remainingFraction: rightRemaining };
  if (rightRemaining === undefined) return { ...left, ...right, remainingFraction: leftRemaining };

  // Production and daily endpoints can expose different snapshots of the same
  // account. Never let a stale 100% response overwrite a lower live value.
  const selected = rightRemaining < leftRemaining ? right : left;
  const other = selected === right ? left : right;
  return {
    ...other,
    ...selected,
    remainingFraction: Math.min(leftRemaining, rightRemaining),
    resetTime: selected.resetTime || other.resetTime,
  };
}

function mergeQuotaSummaryResults(results) {
  const groups = [];
  ...
  for (const result of results) {
    ...
    const summary = isRecord(result.data) ? result.data : {};
    if (!description && typeof summary.description === "string") description = summary.description;

    for (const rawGroup of Array.isArray(summary.groups) ? summary.groups : []) {
      if (!isRecord(rawGroup)) continue;
      const groupKey = String(rawGroup.displayName || rawGroup.groupId || "quota-group").trim().toLowerCase();
      let group = groupsByKey.get(groupKey);
      if (!group) {
        group = { ...rawGroup, buckets: [] };
      ...
      const bucketsByKey = new Map(
        group.buckets.map((bucket, index) => [
          String(bucket.bucketId || bucket.displayName || bucket.id || `bucket-${index}`).trim().toLowerCase(),
          { bucket, index },
        ]),
      );
      for (const rawBucket of Array.isArray(rawGroup.buckets) ? rawGroup.buckets : []) {
        if (!isRecord(rawBucket)) continue;
        const bucketKey = String(rawBucket.bucketId || rawBucket.displayName || rawBucket.id || "quota-bucket").trim().toLowerCase();
        const existing = bucketsByKey.get(bucketKey);
        if (!existing) {
          group.buckets.push(rawBucket);
          bucketsByKey.set(bucketKey, { bucket: rawBucket, index: group.buckets.length - 1 });
        } else {
          group.buckets[existing.index] = mergeQuotaBucket(existing.bucket, rawBucket);
          existing.bucket = group.buckets[existing.index];
        }
      }
    }
  }

  if (!endpoint) throw new Error("/v1internal:retrieveUserQuotaSummary failed: no endpoint available");
  return {
    endpoint,
    status,
...
async function fetchMergedQuotaSummary(token) {
  const results = await Promise.all(
    endpointCandidates().map((endpoint) => fetchQuotaSummaryFromEndpoint(endpoint, token)),
  );
  return mergeQuotaSummaryResults(results);
}

function mergeAvailableModelInfo(left, right) {
  if (!left) return right;
  const merged = { ...left, ...right };
  if (isRecord(left.quotaInfo) || isRecord(right.quotaInfo)) {
    merged.quotaInfo = mergeQuotaBucket(left.quotaInfo || {}, right.quotaInfo || {});
  }
  return merged;
}
...
function inferModelShape(modelId, info = {}) {
  ...
  if (id === "gemini-3.8-flash-tiered") {
    name = "Gemini 3.8 Flash";
  } else if (id === "gemini-3.7-flash-tiered") {
    name = "Gemini 3.7 Flash";
  }
...
function catalogModelStorageShape(model) {
...
function catalogModelsForStorage(models) {
  return (Array.isArray(models) ? models : [])
    .filter((model) => isRecord(model) && typeof model.id === "string" && model.id.length > 0)
    .map(catalogModelStorageShape)
    .sort((a, b) => a.id.localeCompare(b.id));
}

function catalogModelFingerprint(model) {
...
function catalogModelsEqual(left, right) {
  const leftModels = catalogModelsForStorage(left);
  const rightModels = catalogModelsForStorage(right);
  if (leftModels.length !== rightModels.length) return false;
  const rightById = new Map(rightModels.map((model) => [model.id, model]));
  return leftModels.every((model) => {
    const matching = rightById.get(model.id);
    return matching !== undefined && catalogModelFingerprint(model) === catalogModelFingerprint(matching);
  });
}

async function syncModelCatalog(modelSettings, catalogModels) {
  if (!modelSettings) return { changed: false };

  const current = await modelSettings.read();
  const currentCatalog = catalogModelsForStorage(current.catalogModels);
  const nextCatalog = catalogModelsForStorage(catalogModels);
  const currentIds = new Set(currentCatalog.map((model) => model.id));
  const nextIds = new Set(nextCatalog.map((model) => model.id));
  const addedModelIds = [...nextIds].filter((id) => !currentIds.has(id));
  const removedModelIds = [...currentIds].filter((id) => !nextIds.has(id));

  if (catalogModelsEqual(currentCatalog, nextCatalog)) {
    return { changed: false, settings: current, addedModelIds, removedModelIds };
  }

  const currentEnabled = new Set(current.enabledModelIds);
  const allKnownModelsEnabled = currentIds.size === 0
    ? MODELS.every((model) => currentEnabled.has(model.id))
    : [...currentIds].every((id) => currentEnabled.has(id));
  const enabledModelIds = current.enabledModelIds.filter((id) => nextIds.has(id));
  if (allKnownModelsEnabled) {
    for (const id of nextIds) {
      if (!currentEnabled.has(id)) enabledModelIds.push(id);
    }
  }

  const settings = await modelSettings.setCatalogModels(nextCatalog, {
    enabledModelIds: [...new Set(enabledModelIds)],
  });
  return { changed: true, settings, addedModelIds, removedModelIds };
}

async function fetchAndSyncModelCatalog(token, projectId, modelSettings, force = false) {
  const available = await fetchCachedAvailableModels(token, projectId, force);
  const catalogModels = parseCatalogModels(available.data);
  const sync = await syncModelCatalog(modelSettings, catalogModels);
  return { available, catalogModels, sync };
}

async function refreshModelCatalogForStore(store, modelSettings) {
  const { token, projectId } = await ensureApiKey(store);
  return fetchAndSyncModelCatalog(token, projectId, modelSettings);
}
...
export async function fetchAccountQuota(store = new FileCredentialStore(), modelSettings) {
  ...
  const { available, catalogModels, sync: catalogSync } = await fetchAndSyncModelCatalog(
    token,
    projectId,
    modelSettings,
    true,
  ...
  cachedQuota = {
    ...
    catalogModels,
    defaultAgentModelId,
    modelCatalogChanged: Boolean(catalogSync?.changed),
    fetchedAt: Date.now(),
  };
...
function getFallbackRuntimeModel(runtimeModel, effort) {
  ...
  if (runtimeModel === "gemini-3.8-flash") return "gemini-3.7-flash-tiered";
...
function contentToUserParts(content, requestImages = new Map()) {
  ...
  for (const block of content) {
    if (isRecord(block) && block.type === "text") {
      ...
      if (imagePart) {
        ...
        // Keep image-only turns explicit; Antigravity rejects histories ending on a model turn.
        parts.push({ text: version ? requestImageHandleText(version) : "Image attached." });
...
function requestImageHandleText(version) {
  const handle = asString(version?.handle) || asString(version?.id);
  return handle ? `Attached image (${handle}).` : "Image attached.";
}
...
function convertMessages(options, model, runtimeModel, requestImages = new Map()) {
  const contents = [];
...
function buildRequest(options, model, projectId, runtimeModel, effort, requestImages = new Map()) {
  const request = {
  ...
  if (/^gemini-\d+(?:\.\d+)*-flash-tiered$/i.test(runtimeModel)) {
...
function catalogModelsOf(settings) {
  const normalized = normalizeModelSettings(settings);
  if (!normalized.catalogModels.length) return MODELS;

  // Keep newly added static aliases visible before the next catalog refresh.
  // The live catalog still wins for models with the same public ID.
  const models = new Map(MODELS.map((model) => [model.id, model]));
  for (const model of normalized.catalogModels) models.set(model.id, model);
...
async function prepareRequestImages(messages, attachments, signal) {
  const refs = inspectRequestImages(messages);
...
async function* requestAntigravityChunks(options, model, signal, store, attachments) {
  ...
  const imageRefs = containsImages ? inspectRequestImages(options.messages) : new Map();
  if (imageRefs.size && !attachments) {
    throw new LlmError("Antigravity image input requires the durable attachment service", "UNSUPPORTED_CONTENT");
  }
  const requestImages = imageRefs.size ? await prepareRequestImages(options.messages, attachments, signal) : new Map();
...
export class AntigravityAdapter extends LlmAdapter {
  ...
  async listModels(provider) {
    try {
      if (await this.#store.read()) {
        await refreshModelCatalogForStore(this.#store, this.#modelSettings);
      }
    } catch (error) {
      // Model listing must remain available when catalog discovery is offline.
      setLastError(error);
    }
  ...
  // DSH 0.1.1 binds model resolution and dispatch through prepareCall().
  // Keep this explicit so the adapter also works when its local dsh-llm peer
  // is an older release whose LlmAdapter base class has no implementation.
...
function publicModelMatchNeedles(modelId) {
  switch (modelId) {
    case "gemini-3.8-flash":
      return ["gemini-3.8-flash", "gemini 3.8 flash"];
    case "gemini-3.7-flash":
...
function modelOptionsPayload(settings, quota) {
  const enabled = enabledModelSet(settings);
  const modelsById = new Map(catalogModelsOf(settings).map((model) => [model.id, model]));
  for (const model of quota?.catalogModels || []) {
    if (isRecord(model) && typeof model.id === "string" && model.id.length > 0) {
      modelsById.set(model.id, model);
    }
  }
  const allModels = [...modelsById.values()].sort(compareAntigravityModels);
      ...
      () =>
        webCtx.webServer.register({
          ...
          handler: async (request, response) => {
            // Custom routes share the host browser authentication and Host/Origin fence.
            const rejection = webCtx.connection.requestRejection(request);
            if (rejection !== undefined) {
              return sendJson(response, rejection, { ok: false, error: rejection === 401 ? "unauthorized" : "forbidden" });
            }
            ...
            try {
              ...
              if (path === "quota") {
                ...
                // Catalog metadata is persisted only when model IDs/shapes change;
                // quota-only refreshes do not rewrite settings or reload directories.
                const quota = await fetchAccountQuota(store, modelSettings);
                if (quota.modelCatalogChanged) emitLlmAdaptersUpdated(ctx);
...
export function apply(ctx) {
  ctx.inject(["settings"], (settingsCtx) => {
    settingsCtx.settings.register(ANTIGRAVITY_SETTINGS_NS, AntigravitySettingsSchema);
  });
